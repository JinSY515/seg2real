...
class StableDiffusionControlNeXtPipeline(
    ...
    def encode_prompt(
        ...
        elif self.denoising_unet is not None:
            prompt_embeds_dtype = self.denoising_unet.dtype
        else:
    ...
    def enable_freeu(self, s1: float, s2: float, b1: float, b2: float):
        ...
        self.denoising_unet.enable_freeu(s1=s1, s2=s2, b1=b1, b2=b2)

    # Copied from diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion.StableDiffusionPipeline.disable_freeu
    def disable_freeu(self):
        """Disables the FreeU mechanism if enabled."""
        self.denoising_unet.disable_freeu()
    ...
    def do_classifier_free_guidance(self):
        return self._guidance_scale > 1 and self.denoising_unet.config.time_cond_proj_dim is None
    ...
    def __call__(
        ...
        ref_seg_image : PipelineImageInput = None ,
        ...
        **kwargs,
        ...
        ref_seg_image_pil = ref_seg_image 
        ...
        if self.denoising_unet.config.time_cond_proj_dim is not None:
            ...
            timestep_cond = self.get_guidance_scale_embedding(
                guidance_scale_tensor, embedding_dim=self.denoising_unet.config.time_cond_proj_dim
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if mode =="sa_aug"  or mode == "ours":
                    ...
                    noisy_ref_latents = self.scheduler.step(ref_noise_pred, t, noisy_ref_latents, **extra_step_kwargs, return_dict=False)[0]
                ...
                # attn_score_bank = [m.attn_score for m in (torch_dfs(self.denoising_unet.mid_block) + torch_dfs(self.denoising_unet.up_blocks)) if isinstance(m, BasicTransformerBlock) and m.attn_score is not None]
                # # # import pdb; pdb.set_trace()
                # for idx, layer in enumerate(attn_score_bank):
                #     visualize_hacked_attention(attn_score_bank[idx][0], attn_score_bank[idx][1], 
                #                             src_img=None,
                #                             tgt_img=(ref_image),
                #                             query_points=[(int(math.sqrt(attn_score_bank[idx][0].shape[-1])) // 2 , int(math.sqrt(attn_score_bank[idx][0].shape[-1]) // 2)),
                #                                             (int(math.sqrt(attn_score_bank[idx][0].shape[-1])) // 4 , int(math.sqrt(attn_score_bank[idx][0].shape[-1]) // 4))],
                #                             timestep = t,
                #                             layer_idx = idx, 
                #                             is_train=False,
                #                             save_dir=f"attn/1015/train300-5e-6-keywest-maskloss+densecap/whole_train_revised2_fgloss/aug_midup/mat_midup/revised_real_syn1.0/ddim_revised_maskedloss_w1.0/train15005e-7/inf0/{prompt}")    
                # perform guidance
                if self.do_classifier_free_guidance:
