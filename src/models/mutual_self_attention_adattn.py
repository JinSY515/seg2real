            ...
            if self.only_cross_attention:
                ...
                if MODE == "read":
                    ...
                    if len(bank_fea) !=0:
                        ...
                        key = self.attn1.to_k(modify_norm_hidden_states.detach().float())
                        ...
                        query = self.attn1.head_to_batch_dim(query).contiguous()#[0][None]
                        key = self.attn1.head_to_batch_dim(key).contiguous()#[0][None]
                        value = self.attn1.head_to_batch_dim(value).contiguous()#[0][None]
                        ...
                        sim = torch.einsum("b i d, b j d -> b i j", query.detach(), key.detach()) 
                        ...
                        mean = torch.bmm(sim, norm_tgt_features).detach()

                        std = torch.sqrt(torch.relu(torch.bmm(sim, norm_tgt_features ** 2) - mean ** 2)).detach()
                        # print(norm_hidden_states.shape , sim.shape, hidden_states.shape)

                        out = std * query + mean
                        out = rearrange(out, "(b h) n d -> b n (h d)", b=query.shape[0] // self.attn1.heads)
                        del query, key, value, std, mean 
