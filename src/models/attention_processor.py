...
class MatchAttnProcessor(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # if match_embeddings is not None :
        ...
        if self.matcher is not None:
            ...
            # 각 head의 Attention Map 시각화
            # for i in range(8):
            #     visualize_attention(self.attn_map[0, i, :, attn_size//2:], title=f"Head {i+1} Attention")
            

            # seg_corr_resized = F.interpolate(self.seg_corr.float().unsqueeze(1), size=(attn_size//2, attn_size//2), mode='bilinear',align_corners=False)
            # attn_map_resized = 
            # for head in range(self.attn_map.shape[1]):
            #     corr_input = torch.cat([F.interpolate(self.attn_map[:, head, :, attn_size//2:].unsqueeze(1).clone().detach(), size=(32*32, 32*32), mode='bilinear', align_corners=False), self.seg_corr.float().detach().unsqueeze(1)], dim=1)
