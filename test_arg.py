...
h, w = 64, 64
comparison_result = (image1_tensor[:, :, :, None, None] == image2_tensor[:, None, None, :, :]).all(dim=0)
import cv2; cv2.imwrite(f"coords_{(h, w)}.png", comparison_result[:,:, h, w , None].detach().cpu().numpy() * 255)
# 비교 결과를 0 또는 1로 변환하여 out Tensor에 저장
# shape: [256, 256, 256, 256]
comparison_result2 = (image2_tensor[:, :, :, None, None] == image1_tensor[:, None, None, :, :]).all(dim=0)
import cv2; cv2.imwrite(f"coord2_{(h, w)}.png", comparison_result[h, w,:,:, None].detach().cpu().numpy() * 255)

out = comparison_result.to(torch.uint8)
