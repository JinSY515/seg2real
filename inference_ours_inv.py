...
def log_validation(
    ...
    for ref_image_path in ref_image_paths:
        for pose_image_path in pose_image_paths:
            ...
            ref_seg_path = ref_image_path.replace("/gtFine/", "/leftImg8bit/").replace("_gtFine_color", "_leftImg8bit")
            ref_seg_pil = Image.open(ref_seg_path).convert("RGB")
            cond_image_pil = None
        
            os.makedirs(f"/mnt/data3/siyoon/attn_vis/1114_final32_/{mode}/ref_{ref_name}", exist_ok=True)
            save_name = f"/mnt/data3/siyoon/attn_vis/1114_final32_/{mode}/ref_{ref_name}/tgt_{pose_name}"
            image = pipe(
