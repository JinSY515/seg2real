...
if mode == "controlnext":
    ...
    from src.models.mutual_self_attention import ReferenceAttentionControl
    from src.pipelines.pipeline_seg2real_controlnext_ours_inv import StableDiffusionControlNeXtPipeline as Pose2ImagePipeline
elif mode == "ours":
    from src.models.mutual_self_attention import ReferenceAttentionControl
    from src.pipelines.pipeline_seg2real_controlnext_ours_inv import StableDiffusionControlNeXtPipeline as Pose2ImagePipeline
...
def log_validation(
    ...
    import json 
    with open("cityscapes_sim.json", "r") as f :
        data = json.load(f)
    tgt_image_paths = list(data.keys())[:300] # top 300
    dataset = "CityScapes"
    pil_images = []
    ...
    for tgt_name in tqdm(tgt_image_paths):
        ref_name_list = data[tgt_name]
        ref_name = ref_name_list[0]
        if dataset == "CityScapes":
            ref_subdir = ref_name.split("_")[0]
            tgt_subdir = tgt_name.split("_")[0]
            root_path = "/mnt/data1/CityScapes/gtFine/val"
            ref_seg_path = os.path.join(root_path, ref_subdir, ref_name + ".png")
            tgt_seg_path = os.path.join(root_path, tgt_subdir, tgt_name + ".png")
            # import pdb; pdb.set_trace()

            if os.path.exists(ref_seg_path) and os.path.exists(tgt_seg_path):
                ref_seg_pil = Image.open(ref_seg_path).convert("RGB")
                tgt_seg_pil = Image.open(tgt_seg_path).convert("RGB")
                cond_image_pil = None 
                ref_image_path = ref_seg_path.replace("/gtFine/", "/leftImg8bit/").replace("_gtFine_color", "_leftImg8bit")
                ref_image_pil = Image.open(ref_image_path).convert("RGB")
        # elif dataset == "BDD":
        #     root_path = "/mnt/data4/siyoon/bdd100k/bdd100k/labels/10k/sem_seg/colormaps/val"
        #     ref_seg_path = os.path.join(root_path, ref_name + ".png")
        #     tgt_seg_path = os.path.join(root_path, tgt_name + ".png")
        #     # import pdb; pdb.set_trace()
        #     if os.path.exists(ref_seg_path) and os.path.exists(tgt_seg_path):
        #         ref_seg_pil = Image.open(ref_seg_path).convert("RGB")
        #         tgt_seg_pil = Image.open(tgt_seg_path).convert("RGB")
        #         cond_image_pil = None 
        #         ref_image_path = ref_seg_path.replace("/labels/10k/sem_seg/colormaps/", "/images/10k/").replace(".png", ".jpg")
        #         ref_image_pil = Image.open(ref_image_path).convert("RGB")
    ...
    #         cond_image_pil = None
                ...
                os.makedirs(f"/mnt/data3/siyoon/attn_vis/1106_matcher_timestepaware-18000/{mode}/ref_{ref_name}", exist_ok=True)
                save_name = f"/mnt/data3/siyoon/attn_vis/1106_matcher_timestepaware-18000/{mode}/ref_{ref_name}/tgt_{tgt_name}"
                image = pipe(
...
def load_models(cfg):
    ...
    # reference_unet.load_state_dict(torch.load(cfg.net.reference_unet_path))
    # denoising_unet.load_state_dict(torch.load(cfg.net.denoising_unet_path))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/inference/stage1_ours_json.yaml")
